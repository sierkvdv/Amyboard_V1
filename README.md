...
| **MIDI in (TRS)** | BeatStep play/stop = transport (play re-anchors the downbeat). Every incoming note on any channel fires the caught sample at that pitch immediately, velocity-sensitive, even while paused (solo mode) — and it sounds only while held: release fades it out in 150 ms. A white flash strip confirms receipt. MIDI clock (F8) drives the sequencer ticks in lock-step. |
